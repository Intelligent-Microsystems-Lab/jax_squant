...
try:
  ...
  warnings.warn(
      "CUDA-based SQuant is not installed! PyTorch-based SQuant will lead to a prolonged quantization process.")
...
class Quantizer(nn.Module):
  ...
  def adaptive_round(self, x, t_min=None, t_max=None):
    ...
    if self.squant_k:
      ...
      np.save('up.npy', up_priority.detach().cpu().numpy())
      np.save('oo.npy', up_order.detach().cpu().numpy())
      up_priority *= 0.0
      down_priority *= 0.0
    ...
    if self.has_inited_quant_para == False:
      ...
      if self.mode == "squant":
        def _quant(tensor):
          ...
          # np.save('x_min.npy', x_min.detach().cpu().numpy())
          # np.save('x_max.npy', x_max.detach().cpu().numpy())
          scale, zero_point = asymmetric_linear_quantization_params(
              self.bit, x_min, x_max)
          ...
          # import pdb; pdb.set_trace()
          # import pdb; pdb.set_trace()
          # np.save('scale'+name_prefix+'.npy', scale.detach().cpu().numpy())
          # np.save('zero_point.npy', zero_point.detach().cpu().numpy())
          quant_tensor = linear_quantize(
              tensor, scale, zero_point, inplace=False)
  ...
  def _forward(self, data):
    tensor = AsymmetricQuantFunction.apply(
        data, self.bit, self.x_min, self.x_max)
    return tensor
...
class ActivationQuantizer(nn.Module):
  def __init__(self, mode=None, wbit=None, abit=None, args=None):
    ...
    self.quant_output = TensorQuantizer(
        mode=mode, bit=abit, is_signed=False, is_enable=True, args=args, is_input=True)
...
class LinearQuantizer(nn.Module):
  ...
  def __init__(self, mode=None, wbit=None, abit=None, args=None):
    ...
    self.quant_weight = TensorQuantizer(
        mode=mode, bit=wbit, is_signed=True, is_enable=True, args=args)
